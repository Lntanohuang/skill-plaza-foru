#!/usr/bin/env python3
"""Serve the built Vue app and proxy Youcai chat requests without exposing its key."""
...
def origin_allowed(origin: str, server_port: int) -> bool:
    """Allow same-host dev/preview origins: localhost, 127.0.0.1 and LAN addresses."""
    ...
    try:
        ...
        # 局域网内的主机名（如 *.local）也放行，避免预览地址被误拦
...
class SkillPlazaHandler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if origin and not origin_allowed(origin, self.server.server_address[1]):
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the local Skill Plaza app and API proxy.")
    ...
    serving = DIST if DIST.is_dir() else ROOT
    print(f"SKILL 广场：http://{args.host}:{args.port}/（静态目录：{serving}）")
